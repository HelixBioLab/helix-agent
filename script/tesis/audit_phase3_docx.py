#!/usr/bin/env python3
"""Audit F3 after LibreOffice rendering, against the still-installed F2 document."""
import hashlib
import json
import re
import subprocess
...
NEW=Path('/tmp/tesis-fase3-final/Tesis-edited.docx')
...
def main():
    assert hashlib.sha256(SOURCE.read_bytes()).hexdigest()=='b665e59133214ff83144e0c2532fd63226d384ef465ce523f42f54887f81e54b'
    ...
    for i in range(1,18):assert toc.count(f'A.{i}. ')==1
    ...
    assert footers==113
    expected=json.loads((ROOT/'docs/tesis/fase3-documento.json').read_text())
    for section in expected['replace_sections']:
        for block in section['blocks']:assert block['text'] in ps
    for block in expected['appendix']:assert block['text'] in ps
    record={'phase':'F3','source_sha256':hashlib.sha256(SOURCE.read_bytes()).hexdigest(),
        'docx':{'path':str(SOURCE),'sha256':hashlib.sha256(NEW.read_bytes()).hexdigest()},
        'pdf':{'path':'/home/webiwabou/Projects/Tesis-fase3.pdf','sha256':hashlib.sha256(PDF.read_bytes()).hexdigest(),'pages':len(pages),'automatic_section_separator_pages':[i+1 for i,p in enumerate(pages) if not p.strip()]},
        'checks':{'bibliography_paragraphs_preserved':99,'active_figures_preserved':4,'sections_preserved':3,'red_italic_author_notes':len(notes),'r21_table_populated_after_render':True,'five_chapters':True,'chapter_4_and_5_titles_correct':True,'toc_has_21_results_once_each':True,'appendix_sections':17,'toc_duplicates':0,'footer_total_matches_pdf':True,'all_F3_sections_and_appendix_blocks_preserved':True},
        'visual_review_pages':[86,123], 'footer_layout':{'pages_checked':footers,'text_collisions':0,'method':'PDF bounding boxes after reopening the exported DOCX'}}
    (ROOT/'docs/tesis/evidencia/documento-fase3.json').write_text(json.dumps(record,ensure_ascii=False,indent=2)+'\n')

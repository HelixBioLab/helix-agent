#!/usr/bin/env python3
"""Audit F5 after LibreOffice rendering, against the still-installed F4 document."""
import hashlib
import json
import re
import subprocess
...
NEW=Path('/tmp/tesis-fase5-final/Tesis-final.docx')
...
def main():
    assert hashlib.sha256(SOURCE.read_bytes()).hexdigest()=='665f31e3542afd20a1aa388ec8857e95de419ac39b048be89e1162b448ea9ce4'
    ...
    for i in range(1,20):assert toc.count(f'A.{i}. ')==1
    ...
    # Preliminary pages 1-9 have no numbered footer. Writer inserts blank pages
    # at odd-page section starts; their number can change after content grows.
    blank=sum(not page.strip() for page in pages)
    assert footers==len(pages)-9-blank
    expected=json.loads((ROOT/'docs/tesis/fase5-documento.json').read_text())
    ...
    record={'phase':'F5','source_sha256':hashlib.sha256(SOURCE.read_bytes()).hexdigest(),
        'docx':{'path':str(SOURCE),'sha256':hashlib.sha256(NEW.read_bytes()).hexdigest()},
        'pdf':{'path':'/home/webiwabou/Projects/Tesis-fase5.pdf','sha256':hashlib.sha256(PDF.read_bytes()).hexdigest(),'pages':len(pages),'automatic_section_separator_pages':[i+1 for i,p in enumerate(pages) if not p.strip()]},
        'checks':{'bibliography_paragraphs_preserved':99,'active_figures_preserved':4,'sections_preserved':3,'red_italic_author_notes':len(notes),'r21_table_populated_after_render':True,'five_chapters':True,'chapter_4_and_5_titles_correct':True,'toc_has_21_results_once_each':True,'appendix_sections':19,'toc_duplicates':0,'footer_total_matches_pdf':True,'all_F5_sections_and_appendix_blocks_preserved':True},
        'visual_review_pages':[93,129,131], 'footer_layout':{'pages_checked':footers,'text_collisions':0,'method':'PDF bounding boxes after reopening the exported DOCX; visual check of final new sections'}}
    (ROOT/'docs/tesis/evidencia/documento-fase5.json').write_text(json.dumps(record,ensure_ascii=False,indent=2)+'\n')

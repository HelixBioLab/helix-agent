#!/usr/bin/env python3
"""Apply reviewed R3.2 closure to the exact F5 Word revision; write a new DOCX."""
import argparse
import copy
import hashlib
import json
...
EXPECTED = '73a40cea0d532c37c0d20ab7e454aa3d0c546f9350aa08017bbde4bdb7479c9e'
...
def main():
    ...
    content = json.loads((ROOT/'docs/tesis/r32-documento.json').read_text())
    with ZipFile(args.source) as source:
        ...
        for item in content['replace_paragraphs']:
            previous=find(item['old']); replacement=paragraph(item['new'], 'note' if item['new'].startswith('NOTA PARA EL AUTOR.') else 'paragraph')
    ...
    print(json.dumps({'output':str(args.output),'sha256':hashlib.sha256(args.output.read_bytes()).hexdigest(),'phase':'R3.2'},indent=2))

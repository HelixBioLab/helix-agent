#!/usr/bin/env python3
"""Audit F4 after LibreOffice rendering, against the still-installed F3 document."""
import hashlib
import json
import re
import subprocess
...
NEW=Path('/tmp/tesis-fase4-ready/Tesis-final.docx')
...
def main():
    assert hashlib.sha256(SOURCE.read_bytes()).hexdigest()=='0eb83e571276d6069ba3e81265f799eb2d2558fbb3245d753187a23b45e7d962'
    ...
    for i in range(1,19):assert toc.count(f'A.{i}. ')==1
    ...
    expected=json.loads((ROOT/'docs/tesis/fase4-documento.json').read_text())
    ...
    record={'phase':'F4','source_sha256':hashlib.sha256(SOURCE.read_bytes()).hexdigest(),
        'docx':{'path':str(SOURCE),'sha256':hashlib.sha256(NEW.read_bytes()).hexdigest()},
        'pdf':{'path':'/home/webiwabou/Projects/Tesis-fase4.pdf','sha256':hashlib.sha256(PDF.read_bytes()).hexdigest(),'pages':len(pages),'automatic_section_separator_pages':[i+1 for i,p in enumerate(pages) if not p.strip()]},
        'checks':{'bibliography_paragraphs_preserved':99,'active_figures_preserved':4,'sections_preserved':3,'red_italic_author_notes':len(notes),'r21_table_populated_after_render':True,'five_chapters':True,'chapter_4_and_5_titles_correct':True,'toc_has_21_results_once_each':True,'appendix_sections':18,'toc_duplicates':0,'footer_total_matches_pdf':True,'all_F4_sections_and_appendix_blocks_preserved':True},
        'visual_review_pages':[90,96,128], 'footer_layout':{'pages_checked':footers,'text_collisions':0,'method':'PDF bounding boxes after reopening the exported DOCX'}}
    (ROOT/'docs/tesis/evidencia/documento-fase4.json').write_text(json.dumps(record,ensure_ascii=False,indent=2)+'\n')

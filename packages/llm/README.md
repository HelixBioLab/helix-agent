...
Schema-first LLM core for helix. One typed request, response, event, and tool language; provider quirks live in adapters, not in calling code.

```ts
import { Effect } from "effect"
import { LLM, LLMClient } from "@helix/llm"
...
```ts
import { OpenAI, CloudflareAIGateway } from "@helix/llm/providers"

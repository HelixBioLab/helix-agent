...
const envPath = process.env.HELIX_BIN_PATH
const scriptDir = path.dirname(fs.realpathSync(__filename))
const cached = path.join(scriptDir, ".helix-cli")
const platform = { darwin: "darwin", linux: "linux", win32: "windows" }[os.platform()] || os.platform()
const arch = { x64: "x64", arm64: "arm64", arm: "arm" }[os.arch()] || os.arch()
const base = "@helix/cli-" + platform + "-" + arch
const binary = platform === "windows" ? "helix-cli.exe" : "helix-cli"
...
if (!resolved) {
  console.error(
    "It seems that your package manager failed to install the right helix-cli package. Try manually installing " +

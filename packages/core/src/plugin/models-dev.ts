...
export const ModelsDevPlugin = define({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    // The upstream models.dev catalog still lists the hosted "opencode" gateway
    // provider; it has no backing integration in this build, so drop it.
    const skippedProviders = new Set(["opencode"])
    yield* ctx.integration.transform(
      Effect.fn(function* (integrations) {
        const data = yield* modelsDev.get()
        for (const item of Object.values(data)) {
          if (skippedProviders.has(item.id)) continue

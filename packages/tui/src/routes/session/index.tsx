...
import { notifyCopy, useClipboard } from "../../context/clipboard"
  ...
  const sessionCommandList = createMemo(() => [
    {
      ...
      run: async () => {
        const copy = (url: string) =>
          clipboard
            .write?.(url)
            .then((result) => notifyCopy(toast, result, "Share URL copied to clipboard!"))
    ...
    },
    {
      ...
      run: () => {
        ...
        clipboard
          .write?.(text)
          .then((result) => notifyCopy(toast, result, "Message copied to clipboard!"))
    ...
    },
    {
      ...
      run: async () => {
        try {
          ...
          const result = await clipboard.write?.(transcript)
          if (result) notifyCopy(toast, result, "Session transcript copied to clipboard!")
        } catch {

...
import type { LLMClientShape } from "@helix/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("helix"))
    return { type: "unsupported", reason: "provider is not openai, helix, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @helix/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains helix-owned. The native runtime only adapts
      // the @helix/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({

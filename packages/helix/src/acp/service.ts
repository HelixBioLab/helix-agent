...
import type { AssistantMessage, Message, HelixClient, SessionMessageResponse } from "@helix/sdk/v2"
...
export const AuthMethodID = "helix-login"
...
export class Service extends Context.Service<Service, Interface>()("@helix/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `helix auth login` in the terminal",
...
function makeDirectoryService(sdk: HelixClient) {
...
function makeUsageService(sdk: HelixClient) {
...
async function loadDirectorySnapshot(sdk: HelixClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, helix provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const helixProvider = providers[ProviderV2.ID.make("helix")]
  const helixModel = helixProvider ? Provider.sort(Object.values(helixProvider.models))[0] : undefined
  if (helixProvider && helixModel) return { providerID: helixProvider.id, modelID: helixModel.id }

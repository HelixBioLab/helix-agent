// Shared type vocabulary for the direct interactive mode (`helix --mini`).
//
...
import type { HelixClient, PermissionRequest, QuestionRequest, ToolPart } from "@helix/sdk/v2"
import type { TuiConfig } from "@helix/tui/config"
...
type PromptModel = Parameters<HelixClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<HelixClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<HelixClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<HelixClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<HelixClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<HelixClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<HelixClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<HelixClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<HelixClient["question"]["reject"]>[0]

import type { PermissionV1 } from "@helix/core/v1/permission"
import { FSUtil } from "@helix/core/fs-util"
// CLI entry point for `helix run` and `helix --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`helix --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`helix --mini --attach`): connects to a running
//      helix server and runs interactive mode against it.
//
...
import { createHelixClient, type HelixClient, type ToolPart } from "@helix/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<HelixClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run helix with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running helix server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to HELIX_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to HELIX_SERVER_USERNAME or 'helix')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: HelixClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: HelixClient, sessionID: string) {
      ...
      async function current(sdk: HelixClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: HelixClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: HelixClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: HelixClient) {
        ...
        async function loop(client: HelixClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createHelixClient({
        baseUrl: "http://helix.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)

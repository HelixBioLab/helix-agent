...
// `helix dossier [directory]` — collect the four run artefacts into one directory.
//
...
const EXTRA: readonly string[] = [".helix/approvals.jsonl", ".helix/runs/*.json", "census/*"]
...
export const DossierCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.dossier")(function* (args: { directory?: string; out?: string }) {
    ...
    // `helix verify` and the emitted receipt agree on the same project.
    if (!verification.ok) {
      return yield* fail(

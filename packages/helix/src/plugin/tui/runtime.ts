...
import { errorData, errorMessage } from "@helix/tui/util/error"
...
import { hasTheme, upsertTheme } from "@helix/tui/context/theme"
import { Global } from "@helix/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@helix/core/util/flock"
import { Flag } from "@helix/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@helix/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@helix/core/v1/config/plugin"
import { createCommandShim } from "@helix/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@helix/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".helix"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".helix", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".helix", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.HELIX_PURE ? [] : pluginOrigins
    if (Flag.HELIX_PURE && pluginOrigins.length) {
    }

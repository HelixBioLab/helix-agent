...
export const DEPRECATED_PLUGIN_PACKAGES = ["helix-openai-codex-auth", "helix-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, helixVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(helixVersion) || semver.major(helixVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.helix
  if (typeof range !== "string") return
  if (!semver.satisfies(helixVersion, range)) {
    throw new Error(`Plugin requires helix ${range} but running ${helixVersion}`)
  }
}

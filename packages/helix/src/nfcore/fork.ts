...
// release. Only the fork's provenance and its diff vs upstream go in .helix/,
...
export class Service extends Context.Service<Service, Interface>()("@helix/NfcoreFork") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const list = Effect.fn("NfcoreFork.list")(function* () {
      const ctx = yield* InstanceState.context
      const dir = path.join(ctx.directory, HELIX_DIR, "forks")

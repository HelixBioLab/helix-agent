...
//
// State lives project-locally under `.helix/protocol/`, keyed on the project
...
//
//   .helix/protocol/protocol.json     the commitment, written once
//   .helix/protocol/amendments.jsonl  append-only, every signed change to the commitment
//   .helix/protocol/refusals.jsonl    append-only, every attempt that was refused
//
...
// party can read the record without Helix, and `summarize` ships all three into the report.
...
export function directory(projectDir: string): string {
  return path.join(projectDir, HELIX_DIR, PROTOCOL_DIR)
}
  ...
  lines.push(
    ...
    advisory
      ...
      : [
          ...
          `  helix protocol amend ${result.violated[0]?.constraint.id ?? "<constraint-id>"} --action waive --scope "<the request, verbatim>" --reason "<why this once>" --sign "<name>"`,
          "",
    ...
    "This attempt has been appended to .helix/protocol/refusals.jsonl with a timestamp.",
    "</protocol_refusal>",
  )
  return lines.join("\n")
}
...
export class Service extends Context.Service<Service, Interface>()("@helix/NfcoreProtocol") {}

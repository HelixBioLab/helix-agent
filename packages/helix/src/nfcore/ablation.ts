...
// The ablation switch: run the identical model, shell and workspace with Helix's
// specialization layer switched off, so the only thing that differs between two runs is
// the layer itself.
//
// It works because Helix is a fork of opencode: with every layer off, what is
// left is the base coding agent. That only holds if the switch really switches, which is
// why `helix debug ablation` prints hashes rather than claims. A switch nobody
 ...
 * `.unknown` on the floor — so `HELIX_ABLATE=persona,skils` ran with the skills
 * layer ON while the operator believed it was off, and the session was recorded
 * under the wrong configuration with nothing anywhere to say so. Only `helix
  ...
  if (parsed.unknown.length > 0) {
    throw new Error(
      [
        ...
        "under the wrong label. Fix HELIX_ABLATE (or --ablate) and try again.",
      ].join("\n"),
    )
  }
  return parsed.state

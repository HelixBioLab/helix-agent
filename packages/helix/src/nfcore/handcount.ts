...
export const TAXONOMY_VERSION = "helix-handcount-taxonomy/v1"
export const CLASSIFIER_VERSION = "helix-handcount-rules/v1"
...
export interface MethodsContext {
  ...
  /** The agent that did the work, e.g. "Helix 0.1.0 (claude-opus-5)". */
...
export function methods(t: Tally, context: MethodsContext = {}): string {
  ...
  const ledger = context.ledger ?? path.join(HELIX_DIR, FILE)
  const c = t.counts
 ...
 * those are text Helix injected into the user turn (reminders, the restated objective),
...
export function file(directory: string): string {
  return path.join(directory, HELIX_DIR, FILE)
}
...
export class Service extends Context.Service<Service, Interface>()("@helix/NfcoreHandCount") {}
...
export function build(input: RecordInput, countedAt: string): HandCountRecord {
  ...
  const ledger = input.context?.ledger ?? path.join(HELIX_DIR, FILE)

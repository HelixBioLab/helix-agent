...
// Project-local provenance under `.helix/` in the working directory:
//   - `.helix/runs/`         one JSON manifest per pipeline run (history + outcome)
//   - `.helix/approvals.jsonl`  an append-only log of every approval/denial
...
export class Service extends Context.Service<Service, Interface>()("@helix/NfcoreRecord") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // records. Gated here rather than at each write so the bare arm produces no `.helix/`
    ...
    const state = yield* InstanceState.make(
      Effect.fn("NfcoreRecord.state")(function* (ctx) {
        // `.helix/` lives in the working directory Helix was invoked from — the
        // current investigation folder.
        const approvals = path.join(ctx.directory, HELIX_DIR, "approvals.jsonl")
    ...
    const list = Effect.fn("NfcoreRecord.list")(function* () {
      const { directory } = yield* InstanceState.get(state)
      const dir = path.join(directory, HELIX_DIR, "runs")
...
export function summarize(records: readonly RunRecord[]): string {
  if (records.length === 0) return "No recorded runs in .helix/runs/ yet."
  return records

...
// Authoring & contribution backbone. Helix does not
...
export class Service extends Context.Service<Service, Interface>()("@helix/NfcoreAuthoring") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const lint = Effect.fn("NfcoreAuthoring.lint")(function* (input: { directory: string; module?: string }) {
      ...
      const tmp = path.join(os.tmpdir(), `helix-lint-${Date.now()}-${Math.random().toString(36).slice(2)}.json`)

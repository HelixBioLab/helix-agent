...
// nf-core publishes its full pipeline registry as a single JSON document. Helix
...
const USER_AGENT = `helix/${InstallationVersion}`
...
export class Service extends Context.Service<Service, Interface>()("@helix/NfcoreRegistry") {}

export const use = serviceUse(Service)

// Parse nf-core's registry document into just the fields Helix reasons about.
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    /**
     * A catalogue pinned by the operator, via `HELIX_NFCORE_PIPELINES`
    ...
    const loadPinned = Effect.fn("NfcoreRegistry.loadPinned")(function* (manifestPath: string) {
      ...
      if (!manifest) {
        return yield* Effect.die(
          new Error(
            `HELIX_NFCORE_PIPELINES points at ${manifestPath}, which is not a readable snapshot manifest. ` +
      ...
      // Deliberately not re-hashed here: `helix verify` is the thing that checks a
    ...
    const populate = Effect.fn("NfcoreRegistry.populate")(function* () {
      const pinned = process.env["HELIX_NFCORE_PIPELINES"]

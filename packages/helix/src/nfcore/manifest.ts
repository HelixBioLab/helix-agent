...
// Reproducibility manifest. Helix does NOT reinvent provenance
...
// and adds the Helix-native layer those don't capture: the human approval log
...
export interface Manifest {
  ...
  // Helix-native layer (what nf-prov / pipeline_info do not cover):
...
export class Service extends Context.Service<Service, Interface>()("@helix/NfcoreManifest") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const build = Effect.fn("NfcoreManifest.build")(function* (input: { outdir: string; sessionSummary?: string }) {
      ...
      // Helix-native layer: the human approval log.
      const approvalsText = yield* readTextSafe(path.join(ctx.directory, HELIX_DIR, "approvals.jsonl"))
      ...
      // Correlate the most recent Helix run record for this outdir.
      const runsDir = path.join(ctx.directory, HELIX_DIR, "runs")

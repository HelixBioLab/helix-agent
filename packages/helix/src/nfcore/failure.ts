...
// When a Nextflow run fails, Helix follows a fixed sequence:
...
export const Diagnosis = Schema.Struct({
  ...
  // Can Helix reasonably fix this itself (config, dependency, resource, input)?
...
export function classify(text: string, exitCode?: number): Diagnosis {
  ...
  if (has(t, /connection timed out|could not resolve host|failed to download|read timed out|temporary failure in name resolution|unable to fetch|502 bad gateway|connection refused|network is unreachable/)) {
    return {
      ...
      explanation:
        "The failure is an external issue — a download failed, a host couldn't be resolved, or a service was unreachable. This is outside what Helix can fix.",
      reparable: false,
      resumable: true,
      suggestedFix:
        "This is an external network/service problem, not something Helix can repair. Check connectivity or try again later; Nextflow can then re-run with -resume to continue where it stopped.",
    }
  }
...
export class Service extends Context.Service<Service, Interface>()("@helix/NfcoreFailure") {}
...
export function summarize(d: Diagnosis): string {
  return [
    ...
    "",
    d.reparable
      ? "Helix can likely fix this with your approval."
      : "This is beyond what Helix can safely fix on its own — be honest about that.",

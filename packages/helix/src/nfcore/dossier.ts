...
// visible, `.helix/` hidden, the census only in memory — and none referenced any of the
...
export const LIMITS: readonly Limit[] = [
  {
    ...
    evidence: "packages/helix/src/nfcore/verify.ts",
  ...
  },
  {
    ...
    evidence: "packages/helix/src/nfcore/record.ts",
  ...
  },
  {
    ...
    evidence: "packages/helix/src/nfcore/protocol.ts",
  ...
  },
  {
    ...
    evidence: "packages/helix/src/nfcore/manifest.ts",
  ...
  },
  {
    ...
    evidence: "packages/helix/src/nfcore/handcount.ts",
  ...
  },
  {
    ...
    evidence: "packages/helix/src/nfcore/dossier.ts",
...
export function verifyScript(index: Index, digest: string): string {
  const lines = [
    "#!/bin/sh",
    "# Check this dossier without Helix, without a model and without a network.",
    "#",
    "# Generated by `helix dossier`. It re-computes the same two things `helix verify`",
    ...
    "# and a differential test asserts this script and `helix verify` agree.",
    "",
    "set -eu",
    'cd "$(dirname "$0")"',
    "",
...
export function format(index: Index, digest: string): string {
  ...
  out.push("Run `sh verify.sh` to check it without Helix installed.")
  return out.join("\n")
}

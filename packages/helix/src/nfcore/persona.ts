...
// Built-in nf-core skills that ship with Helix. These are scaffold-level guidance
...
export const BuiltinSkills: readonly BuiltinSkill[] = [
  ...
  },
  {
    ...
    description:
      "How Helix Agent inspects and repairs the local environment for nf-core runs: Nextflow version, Docker/conda backend, CPU/RAM/GPU resources, resource-margin adaptation, and the self-install vs. show-the-sudo-command policy. Load this when diagnosing environment, version, or resource problems.",
    content: SKILL_ENVIRONMENT,
  },
  {
    name: "nfcore-samplesheet",
    description:
      "How Helix Agent builds an nf-core samplesheet from a folder of data: reading the pipeline's own schema_input.json for the real columns, inspecting files by name/header only, mapping files to samples, asking when grouping or read pairing is ambiguous, validating, and writing the CSV. Load this when creating or fixing a samplesheet.",
    content: SKILL_SAMPLESHEET,
  },
  {
    name: "nfcore-run",
    description:
      "How Helix Agent runs an nf-core pipeline: building the exact nextflow run command, running the test profile first through the shell tool with approval, then the real data. Load this when executing, testing, or launching a pipeline run.",
    content: SKILL_RUN,
  },
  {
    name: "nfcore-failure",
    description:
      "How Helix Agent recovers when a Nextflow run fails: diagnose the real cause from the logs, explain it, offer a specific fix with approval, suggest -resume rather than restarting, and stop honestly when the cause is beyond its reach. Load this when a run fails, errors out, or crashes.",
    content: SKILL_FAILURE,
  },
  {
    name: "nfcore-manifest",
    description:
      "How Helix Agent builds a reproducibility manifest after a run: referencing nf-core's pipeline_info/ and nf-prov (BCO/RO-Crate) provenance without reinventing it, plus the human-approval and session-summary layer Helix Agent adds on top. Load this when a real run finishes or when the scientist asks about reproducibility or provenance.",
    content: SKILL_MANIFEST,
  },
  {
    name: "literature-synthesis",
    description:
      "How Helix Agent sources claims from the biomedical literature and databases: searching PubMed (pubmed_search/pubmed_fetch) as the primary path, citing every source, using general web search only for gaps, and distinguishing cited vs computed vs model-inferred claims. Load this whenever a statement needs a literature or database source.",
    content: SKILL_LITERATURE,
  },
  {
    name: "claim-typing",
    description:
      "How Helix Agent tags every statement in a saved report as [computed], [cited], or [model-inferred] so a reader knows what backs it, keeps those tags out of ordinary conversation, and asks where to save each report. Load this when writing, interpreting, or saving a report or written analysis with report_save.",
    content: SKILL_CLAIM_TYPING,
  },
  {
    name: "result-critique",
    description:
      "How Helix Agent runs its adaptive critique pass after each result: calling nfcore_critique for the rigor concerns shaped to the analysis type (multiple testing, batch/confounders, replication/power, assumptions), flagging without blocking, and soft-blocking only when an analysis is invalid as configured. Load this when interpreting, summarizing, or judging the validity of any analysis result.",
    content: SKILL_CRITIQUE,
  },
  {
    name: "nfcore-authoring",
    description:
      "How Helix Agent helps author new nf-core modules, subworkflows, and pipelines: orchestrating the official nf-core scaffolders (nf-core modules/subworkflows/pipelines create), filling in the scientist's content, and verifying with nf-test and nf-core lint — never hand-writing boilerplate and never reimplementing the underlying science. Load this when creating, adapting, testing, or preparing to contribute an nf-core module, subworkflow, or pipeline.",
    content: SKILL_AUTHORING,
  },
  {
    name: "nfcore-contribution",
    description:
      "How Helix Agent helps a scientist contribute a module or pipeline back to nf-core, or publish their own: making it conformant via nfcore_lint (zero failures) and nf-test, then the fork/branch/PR flow and conventions, and publishing via the nf-core pipelines create template — never free-form pipeline code, and never claiming a PR was opened unless it was. Load this when preparing a contribution, a pull request, or publishing a pipeline.",
    content: SKILL_CONTRIBUTION,
  },
  {
    name: "nfcore-fork",
    description:
      "How Helix Agent forks a pipeline for local code changes: cloning it into a visible pipelines/<name>-fork/ folder pinned to one upstream release, recording provenance in .helix/ via nfcore_fork, keeping it conformant with nfcore_lint, and tracking exactly how it diverged from upstream with nfcore_fork_status. Load this when a scientist needs to modify a pipeline's code rather than just its parameters.",
    content: SKILL_FORK,
  },
  {

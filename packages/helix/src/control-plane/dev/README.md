This is a plugin to simulate a remote environment locally. Add this to `.helix/helix.jsonc`:

```json
  "plugin": ["../packages/helix/src/control-plane/dev/debug-workspace-plugin.ts"],
```
...
```
./packages/helix/script/run-workspace-server
```

...
// tool ("helix") and a contact email; an optional api_key raises the rate limit
// and is read from the environment or ~/.config/helix/ncbi.json.

const BASE = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils"
const TOOL = "helix"
const DEFAULT_EMAIL = "helix-agent@users.noreply.github.com"
...
export class Service extends Context.Service<Service, Interface>()("@helix/Entrez") {}
...
async function loadCredentials(): Promise<Credentials> {
  let email = process.env.HELIX_NCBI_EMAIL
  let apiKey = process.env.HELIX_NCBI_API_KEY
  if (!email || !apiKey) {
    try {

...
export const NfcoreForkTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      // Planning the clone command and recording provenance into .helix/ is bookkeeping,
        ...
        Effect.gen(function* () {
          ...
          if (result.kind === "plan") {
            return {
              ...
              output: `To fork nf-core/${params.pipeline} at ${params.release}, run this through the shell tool (it writes files, so it asks for approval):\n\n${result.plan.command}\n\nThe fork lands in ${result.plan.path} (a visible project folder — the scientist's code, not hidden in .helix). After it clones, call nfcore_fork again to record its provenance.`,
            }
          }
          return {
            title: `${params.pipeline}@${params.release}`,

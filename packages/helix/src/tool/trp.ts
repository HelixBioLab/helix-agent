        ...
        output: JSON.stringify(
          {
            ...
            scope:
              "Legacy PDB, explicit model and author chain, positive author residue intervals without insertion codes or alternate locations. Optional coordinateMapping derives author units from matching mmCIF labels or archived residue-level SIFTS/UniProt mappings. GeomeTRe requires at least three supplied units. Experimental B-factor is not pLDDT.",
          },
          null,
          2,
        ),
...
export const TrpPrepareTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    return {
      description:
        "Validate a complete TRP specification against actual PDB bytes and catalogue evidence, and prepare immutable Nextflow code for review. Each parameter requires a declared source. Every edge must explicitly agree on protein, chain, format, numbering, insertion codes, confidence and scale. Optional coordinateMapping supplies hashed mmCIF plus authoritative label_seq_id intervals, or UniProt intervals with a hashed residue-level SIFTS XML archive and accession. Preparation verifies every residue identity and coordinate against the PDB and derives author units automatically. Original sources, mappings and versions are bound to approval and re-read after approval. Existing insertion intervals remain in author numbering. Use pdb/xyz:angstrom;B:angstrom^2 for structure, json/author-residue-interval for units, pdb-author numbering and none confidence. Source structure preserves insertion codes; checked selection and units exclude them. Replaces any previous draft in this session. Supply an explicit budget (CPU, memoryBytes, workBytes, wallSeconds, requireHardStorageLimit and its origin) for human review; omission is a typed refusal. Inventory is observed by the host, never supplied by the model. Storage is estimated; a hard quota requires a verified operator XFS/cgroup allocation. Each such allocation permits one preparation; a replacement needs a fresh allocation. Does not execute scientific tasks. If invalid, report failed checks and clarify missing information with the user.",
      parameters: Schema.Struct({

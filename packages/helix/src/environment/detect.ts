...
// A read-only snapshot of everything Helix needs to know before it can run an
...
// Which container backend Helix would actually use, resolved from what's
...
export class Service extends Context.Service<Service, Interface>()("@helix/Environment") {}
...
export function summarize(report: Report): string {
  ...
  const lines = [
    ...
    `- Container backend Helix would use: ${report.containerBackend}`,

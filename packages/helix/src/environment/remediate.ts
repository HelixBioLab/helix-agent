...
//   - "install": installable without root. Helix runs `command` for the scientist
//                through the shell tool, after showing it and getting approval.
//   - "manual":  needs root (or a human decision). Helix shows `command` and the
//                scientist runs it. Helix never runs a privileged command itself.
...
export function plan(report: Environment.Report): Plan {
  ...
  if (report.platform.os === "windows") {
    ...
    steps.push({
      ...
      rationale: available
        ? "Nextflow needs a POSIX environment, so the work happens inside the WSL distribution and not here. Install Helix inside it with the Windows installer at https://helixbiolab.github.io/helix-agent/install.ps1, which also leaves a launcher on the Windows PATH that carries the current directory across."
        : "Nextflow needs a POSIX environment, and Windows already ships one: WSL. `wsl --install` needs administrator rights and usually a restart. The Windows installer at https://helixbiolab.github.io/helix-agent/install.ps1 walks through it and asks before elevating anything.",
    })
  ...
  if (report.nextflow.installed) {
    ...
    steps.push({
      ...
      rationale:
        "Neither Java nor conda was found. Install micromamba (no root) first, then re-run detection so Helix can install Nextflow, Java, and nf-core tools through it.",
    })
  }
  ...
  if (report.nfcoreTools.installed) {
    ...
    steps.push({
      ...
      rationale: "The nf-core CLI is installed via conda without root; Helix relies on it for pipeline metadata and schemas.",
    })
  } else {
    steps.push({
      dependency: "nf-core tools",
      ...
      rationale: "The nf-core CLI is installed with a per-user pip install (no root); Helix relies on it for pipeline metadata and schemas.",
    })
  }
...
export function summarize(plan: Plan): string {
  ...
  for (const step of plan.steps) {
    ...
    if (step.action === "install") {
      lines.push(`- ${step.dependency}: ${step.status}. Helix can install this for you (no root):`)

...
import { isRecord } from "@helix/tui/util/record"
...
import { TuiKeybind } from "@helix/tui/config/keybind"
...
export class Service extends Context.Service<Service, Interface>()("@helix/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old helix.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.helix`
  // folders between cwd and home, and HELIX_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.HELIX_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit HELIX_TUI_CONFIG override, if set.
  if (Flag.HELIX_TUI_CONFIG) {
    const configFile = Flag.HELIX_TUI_CONFIG
  ...
  // 4. `.helix` directories (and HELIX_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".helix") || dir === Flag.HELIX_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".helix") && dir !== Flag.HELIX_CONFIG_DIR) continue

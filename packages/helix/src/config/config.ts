...
import { InstallationLocal, InstallationVersion } from "@helix/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@helix/core/v1/config/console-state"
...
import { RemoteAuthError } from "@helix/core/v1/config/error"
import { ConfigPermissionV1 } from "@helix/core/v1/config/permission"
...
// Skip schema stamping until a config schema is published at a stable URL.
const CONFIG_SCHEMA_URL: string | undefined = undefined
...
export class Service extends Context.Service<Service, Interface>()("@helix/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["helix.jsonc", "helix.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema && CONFIG_SCHEMA_URL) {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.HELIX_CONFIG && !Flag.HELIX_CONFIG_DIR && !Flag.HELIX_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          // Seed the file either way so there is somewhere obvious to edit; stamp
          // the schema only when one is actually published.
          yield* fs
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "helix.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "helix.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "HELIX_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/helix`
            ...
            if (!remoteConfig.$schema && CONFIG_SCHEMA_URL) remoteConfig.$schema = CONFIG_SCHEMA_URL
        ...
        if (Flag.HELIX_CONFIG) {
          yield* merge(Flag.HELIX_CONFIG, yield* loadFile(Flag.HELIX_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.HELIX_CONFIG })
        }

        if (!Flag.HELIX_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("helix", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.HELIX_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from HELIX_CONFIG_DIR", { path: Flag.HELIX_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".helix") || dir === Flag.HELIX_CONFIG_DIR) {
            for (const file of ["helix.json", "helix.jsonc"]) {
          ...
          // Auto-discovered plugins under `.helix/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.HELIX_CONFIG_CONTENT) {
          const source = "HELIX_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.HELIX_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from HELIX_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["HELIX_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("HELIX_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.HELIX_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.HELIX_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("HELIX_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.HELIX_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.HELIX_DISABLE_PRUNE) {

...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from helix.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const helix = await helixFiles(input)
  for (const file of helix) {
...
async function helixFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "helix"),
    ...(await Filesystem.findUp(["helix.json", "helix.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "helix"))
  }
  if (Flag.HELIX_CONFIG) files.push(Flag.HELIX_CONFIG)

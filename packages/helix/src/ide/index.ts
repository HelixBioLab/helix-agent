...
import { IdeEvent } from "@helix/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["HELIX_CALLER"] === "vscode" || process.env["HELIX_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.helix"], {
    nothrow: true,
  })

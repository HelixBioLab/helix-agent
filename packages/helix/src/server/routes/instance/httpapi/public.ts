...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for helix serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(HelixHttpApi)
...
export const PublicApi = HelixHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "helix",
    version: "1.0.0",
    description: "helix api",

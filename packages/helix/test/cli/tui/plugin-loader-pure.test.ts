...
`,
  ...
  const pure = process.env.HELIX_PURE
  const meta = process.env.HELIX_PLUGIN_META_FILE
  process.env.HELIX_PURE = "1"
  process.env.HELIX_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.HELIX_PURE
    } else {
      process.env.HELIX_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.HELIX_PLUGIN_META_FILE
    } else {
      process.env.HELIX_PLUGIN_META_FILE = meta
    }
  }
})

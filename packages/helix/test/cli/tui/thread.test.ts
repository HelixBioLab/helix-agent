...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ helix }) =>
    Effect.gen(function* () {
      const result = yield* helix.spawn(["--replay-limit", "10"])

      helix.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ helix }) =>
    Effect.gen(function* () {
      const result = yield* helix.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ helix }) =>
    Effect.gen(function* () {
      const result = yield* helix.spawn(["--mini", "--port", "4096"])

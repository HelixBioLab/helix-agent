...
describe("helix read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // HELIX_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ helix }) =>
      Effect.gen(function* () {
        const r = yield* helix.spawn(["mcp", "list"])
        helix.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ helix }) =>
      Effect.gen(function* () {
        const r = yield* helix.spawn(["providers", "list"])
        helix.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ helix }) =>
      Effect.gen(function* () {
        const r = yield* helix.spawn(["models"])
        helix.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ helix }) =>
      Effect.gen(function* () {
        const r = yield* helix.spawn(["agent", "list"])
        helix.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh HELIX_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ helix }) =>
      Effect.gen(function* () {
        const r = yield* helix.spawn(["session", "list"])
        helix.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ helix }) =>
      Effect.gen(function* () {
        const r = yield* helix.spawn(["stats"])
        helix.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under HELIX_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ helix }) =>
      Effect.gen(function* () {
        const r = yield* helix.spawn(["db", "path"])
        helix.expectExit(r, 0, "db path")

...
describe("helix acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ helix }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ helix }))
        ...
        expect(initialized.agentInfo?.name).toBe("Helix")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ helix }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ helix })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("helix-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "helix-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.HELIX_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ helix }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("helix-login")

// Subprocess integration tests for `helix run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `helix.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `HELIX_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("helix run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, helix }) =>
      Effect.gen(function* () {
        ...
        const result = yield* helix.run("say hi")
        helix.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, helix }) =>
      Effect.gen(function* () {
        ...
        const result = yield* helix.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, helix }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* helix.run("think", { extraArgs: ["--thinking"] })
        helix.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* helix.run("think again")
        helix.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, helix }) =>
      Effect.gen(function* () {
        ...
        const result = yield* helix.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, helix }) =>
      Effect.gen(function* () {
        ...
        const result = yield* helix.run("say hi", { format: "json" })
        helix.expectExit(result, 0)

        const events = helix.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ helix }) =>
      Effect.gen(function* () {
        const result = yield* helix.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, helix }) =>
      Effect.gen(function* () {
        ...
        const result = yield* helix.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, helix }) =>
      Effect.gen(function* () {
        ...
        const result = yield* helix.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, helix }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* helix.run("request permission", { permission: { bash: "ask" } })
        helix.expectExit(denied, 0)
        ...
        const allowed = yield* helix.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        helix.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* helix.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        helix.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, helix }) =>
      Effect.gen(function* () {
        ...
        const server = yield* helix.serve()

        const result = yield* helix.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, helix }) =>
      Effect.gen(function* () {
        const result = yield* helix.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, helix }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* helix.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()

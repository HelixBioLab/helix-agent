...
import { HelixClient, type GlobalEvent } from "@helix/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<HelixClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<HelixClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<HelixClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<HelixClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<HelixClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://helix.test"),
...
function sdk(
  input: {
    ...
    subscribe?: HelixClient["event"]["subscribe"]
    globalEvent?: HelixClient["global"]["event"]
    promptAsync?: HelixClient["session"]["promptAsync"]
    status?: HelixClient["session"]["status"]
    messages?: HelixClient["session"]["messages"]
    children?: HelixClient["session"]["children"]
    permissions?: HelixClient["permission"]["list"]
    questions?: HelixClient["question"]["list"]
  } = {},
) {
  const client = new HelixClient()

  const subscribe: HelixClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: HelixClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: HelixClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: HelixClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: HelixClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: HelixClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: HelixClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: HelixClient["question"]["list"] = input.questions ?? (() => ok([]))

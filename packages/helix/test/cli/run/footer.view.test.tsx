...
import type { QuestionRequest } from "@helix/sdk/v2"
import { HelixKeymapProvider, registerHelixKeymap } from "@helix/tui/keymap"
import {
...
function model(input: {
  ...
  return {
    ...
    api: {
      id: "helix",
      url: "https://helixbiolab.github.io/helix-agent",
...
async function renderFooter(
  ...
  function Harness() {
    ...
    offKeymap = registerHelixKeymap(keymap, renderer, config)
...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    offKeymap = registerHelixKeymap(keymap, renderer, tuiConfig)
...
test("direct footer separates a lone context hint from model and command hint", async () => {
  const app = await renderFooter({
    providers: [provider()],
    currentModel: { providerID: "helix", modelID: "gpt-5" },
...
test.skip("direct custom answer submits through keymap return binding", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    off = registerHelixKeymap(keymap, renderer, tuiConfig)
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "helix", modelID: "gpt-5" })

...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "helix.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "helix.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "helix.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "helix.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["helix.json", "helix.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "helix.json"),
        path.join(project, "helix.jsonc"),
      ])
    })
  })

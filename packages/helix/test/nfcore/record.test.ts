...
describe("nfcore.record", () => {
  it.live("logs a granted approval to .helix/approvals.jsonl", () =>
    provideTmpdirInstance((dir) =>
      Effect.gen(function* () {
        ...
        const log = yield* readSafe(path.join(dir, ".helix", "approvals.jsonl"))
  ...
  it.live("records a run to .helix/runs/ and lists it back", () =>
    provideTmpdirInstance((dir) =>
      Effect.gen(function* () {
        const record = yield* Record.Service
        ...
        expect(file).toContain(path.join(dir, ".helix", "runs"))

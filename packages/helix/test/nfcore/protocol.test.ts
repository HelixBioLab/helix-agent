...
describe("nfcore.protocol paths", () => {
  test("the commitment and its ledgers live project-locally under .helix/protocol/", () => {
    expect(Protocol.file("/work/proj")).toBe(path.join("/work/proj", ".helix", "protocol", "protocol.json"))
    expect(Protocol.refusalsFile("/work/proj")).toBe(
      path.join("/work/proj", ".helix", "protocol", "refusals.jsonl"),
    )
...
describe("nfcore.protocol service", () => {
  it.live("a refusal is written to the ledger and survives a session restart", () =>
    provideTmpdirInstance((dir) =>
      Effect.gen(function* () {
        ...
        const ledgerPath = path.join(dir, ".helix", "protocol", "refusals.jsonl")
  ...
  it.live("proceeding takes a signed amendment; an unsigned one is rejected", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const protocol = yield* Protocol.Service
        yield* protocol.commit({ statement: "s", constraints: [CALIBRATION], committedBy: "R. Helix" })
        ...
        expect(
          yield* tagOf(
            protocol.amend({ action: "waive", constraintId: "calibration", reason: "deadline", signedBy: "R. Helix" }),
          ),
        ...
        expect(ledger.amendments[0].signedBy).toBe("R. Helix")
      }),
    ),
  )
  ...
  it.live("a protocol file that exists but does not decode fails loudly instead of unbinding the campaign", () =>
    provideTmpdirInstance((dir) =>
      Effect.gen(function* () {
        const target = path.join(dir, ".helix", "protocol", "protocol.json")
  ...
  it.live("nothing is written, and nothing refused, before a protocol is committed", () =>
    provideTmpdirInstance((dir) =>
      Effect.gen(function* () {
        ...
        expect(yield* readSafe(path.join(dir, ".helix", "protocol", "refusals.jsonl"))).toBe("")

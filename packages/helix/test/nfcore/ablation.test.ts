...
describe("nfcore.ablation tool classification", () => {
  ...
  test("base coding-agent tools are never ablated", () => {
    // These are opencode's, not Helix's. Removing them would make the bare
...
describe("nfcore.ablation leak check", () => {
  ...
  test("a bare prompt still carrying the persona is a leak", () => {
    const found = Ablation.leaks({
      state: Ablation.BARE,
      prompt: "You are Helix, a bioinformatics co-scientist.",
      personaMarker: "You are Helix",
      skillNames: skills,
      toolNames: tools,
    })
  ...
  test("nothing is a leak when the layer is ON", () => {
    ...
    expect(
      Ablation.leaks({
        state: Ablation.FULL,
        prompt: "You are Helix. use discovery-campaign and nfcorePipeline",
        personaMarker: "You are Helix",
        skillNames: skills,
        toolNames: tools,
      }),

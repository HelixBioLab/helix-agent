...
// The dossier's whole claim is that someone without Helix can check it, so the
...
const index = (entries: Dossier.Entry[]): Dossier.Index => ({
  ...
  taxonomyVersion: "helix-handcount-taxonomy/v1",
  classifierVersion: "helix-handcount-rules/v1",
  artefacts: [],
...
function fixture(files: Record<string, string>) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "helix-dossier-"))
...
/** Run the receipt the way a reviewer would: /bin/sh, no Helix, no environment. */
...
describe("nfcore.dossier receipt", () => {
  test("an intact dossier verifies with no Helix and no network", () => {

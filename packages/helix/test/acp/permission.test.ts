...
import type { Event, HelixClient } from "@helix/sdk/v2"
...
type PermissionReplyParams = Parameters<HelixClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "helix-acp-permission-"))

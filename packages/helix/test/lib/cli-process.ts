// Subprocess test harness for the helix CLI. Spawns the real binary against
...
//
// Configuration flows through helix's built-in test affordances:
//   - HELIX_CONFIG_CONTENT      : provider config inline, no files to find
//   - HELIX_TEST_HOME           : pins os.homedir() → tmpdir
//   - HELIX_DISABLE_PROJECT_CONFIG : skip walking up for helix.json
//   - HELIX_PURE                : skip external plugin discovery + install
//   - HELIX_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `helix.run` is fully wired. The shape supports adding more
// builders (`helix.serve(opts)`, `helix.acp(opts)`, `helix.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of HelixCli.
...
const helixRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(helixRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    HELIX_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    HELIX_CONFIG_CONTENT: configJson,
    HELIX_DISABLE_PROJECT_CONFIG: "1",
    HELIX_PURE: "1",
    HELIX_DISABLE_AUTOUPDATE: "1",
    HELIX_DISABLE_AUTOCOMPACT: "1",
    HELIX_DISABLE_MODELS_FETCH: "1",
    HELIX_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `helix run`. New flags should
...
// `helix serve` is a long-lived process — it never exits on its own.
...
// `helix acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type HelixCli = {
  ...
  // Spawn `helix serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `helix acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("helix.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          HELIX_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("helix.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("helix.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "helix server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `helix serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("helix.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const helix: HelixCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, helix })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `helix.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "helix") {
...
// every call site. Use this for any test that needs the helix CLI fixture.
//
...
// resources (e.g. `helix.serve`) without an extra `Effect.scoped` wrapper —

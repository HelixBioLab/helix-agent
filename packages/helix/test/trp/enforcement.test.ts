...
describe("operator resource allocation", () => {
  ...
  test("untrusted state path cannot assert kernel limits", async () => {
    const old = process.env.HELIX_TRP_SANDBOX
    process.env.HELIX_TRP_SANDBOX = "/tmp/pretend-root.json"
    try { await assert.rejects(E.observe("/tmp"), /Invalid supervisor state path/) }
    finally {
      if (old === undefined) delete process.env.HELIX_TRP_SANDBOX
      else process.env.HELIX_TRP_SANDBOX = old
    }
  })

...
it.instance("plan agent denies edits except .helix/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".helix/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".helix", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.HELIX_TEST_HOME
      process.env.HELIX_TEST_HOME = test.directory

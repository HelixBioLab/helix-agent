  ...
  Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("HELIX_SERVER_PASSWORD", original.envPassword)
        restoreEnv("HELIX_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            HELIX_SERVER_PASSWORD: input?.password,
            HELIX_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
...
describe("HttpApi UI fallback", () => {
  it.live("returns 404 without contacting any upstream when no web UI is bundled", () =>
    Effect.gen(function* () {
      let proxiedUrl: string | undefined

      const response = yield* uiApp({
        disableEmbeddedWebUi: true,
        client: httpClient(new Response("should never be requested"), (request) => {
          proxiedUrl = request.url
        }),
      }).request("/")
  ...
  it.live("accepts auth token for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        username: "helix",
        disableEmbeddedWebUi: true,
      }).request(`/?auth_token=${btoa("helix:secret")}`)
  ...
  it.live("accepts basic auth for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        headers: { authorization: `Basic ${btoa("helix:secret")}` },
      })
  ...
  it.live("accepts basic auth passwords containing colons for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        headers: { authorization: `Basic ${btoa("helix:sec:ret")}` },
      })
  ...
  // The browser fetches the PWA manifest and its icons via flows that don't
  // carry app-managed credentials (the `<link rel="manifest">` request is not
  // under page-auth control), so the server returning 401 breaks PWA install.
  // These specific public assets should bypass auth.
  ...
  it.live("allows web UI preflight without auth", () =>
    Effect.gen(function* () {
      const response = yield* app({ password: "secret", username: "helix" }).request("/", {
        method: "OPTIONS",
        headers: {

...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-helix-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-helix-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://helixbiolab.github.io/helix-agent/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-helix-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-helix-workspace")

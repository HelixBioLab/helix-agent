...
const original = {
  ...
  envPassword: process.env.HELIX_SERVER_PASSWORD,
  envUsername: process.env.HELIX_SERVER_USERNAME,
}
const auth = { username: "helix", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.HELIX_SERVER_PASSWORD = original.HELIX_SERVER_PASSWORD
  Flag.HELIX_SERVER_USERNAME = original.HELIX_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.HELIX_SERVER_PASSWORD
  else process.env.HELIX_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.HELIX_SERVER_USERNAME
  else process.env.HELIX_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.HELIX_SERVER_PASSWORD = auth.password
  Flag.HELIX_SERVER_USERNAME = auth.username
  process.env.HELIX_SERVER_PASSWORD = auth.password
  process.env.HELIX_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.HELIX_SERVER_PASSWORD = undefined
  Flag.HELIX_SERVER_USERNAME = auth.username
  delete process.env.HELIX_SERVER_PASSWORD
  process.env.HELIX_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-helix-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-helix-ticket": "1" }),
...
describe("HttpApi Server.listen", () => {
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-helix-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  test("default in-process handler does not emit Effect HTTP response logs", async () => {
    ...
    try {
      // /status has no API route; without a bundled web UI the fallback serves 404,
      // which still exercises the response path this test captures logs for.
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.HELIX_DISABLE_DEFAULT_PLUGINS
    process.env.HELIX_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.HELIX_DISABLE_DEFAULT_PLUGINS
      else process.env.HELIX_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-helix-ticket": "1" },
      })

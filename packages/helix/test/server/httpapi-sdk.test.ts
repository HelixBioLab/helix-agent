...
import { createHelixClient } from "@helix/sdk/v2"
...
type Sdk = ReturnType<typeof createHelixClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.HELIX_SERVER_PASSWORD = input?.password
      Flag.HELIX_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".helix", "skills", "project-rest-skill", "SKILL.md"),
      `---
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(found.data?.data).toEqual(expect.arrayContaining([{ path: "hello.txt", type: "file" }]))
        ...
        expect(request!.headers.has("x-helix-directory")).toBe(false)
        expect(request!.headers.has("x-helix-workspace")).toBe(false)
      }),
    ),
  )
    ...
    ({ directory }) =>
      Effect.gen(function* () {
        ...
        const badSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("helix", "wrong") },
        })
        const bad = yield* capture(() => badSdk.file.read({ path: "hello.txt" }))
        const goodSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("helix", "secret") },
        })

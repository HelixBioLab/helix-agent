...
describe("session.retry.retryable", () => {
  ...
  test("carries no subscription upsell on provider usage limits", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
    ...
    // The error still retries, but it carries the provider's own message and no
    // action: this project sells no subscription to point the user at.
    expect(SessionRetry.retryable(error, retryProvider)).toEqual({ message: "Free usage exceeded" })
  })
})

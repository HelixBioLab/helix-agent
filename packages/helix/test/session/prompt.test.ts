...
import { SessionV1 } from "@helix/core/v1/session"
...
import { SessionMessageTable } from "@helix/core/session/sql"
...
import { Ripgrep } from "@helix/core/ripgrep"
...
import { LocationServiceMap, locationServiceMapLayer } from "@helix/core/location-services"
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<ConfigV1.Info>) {
  yield* writeText(
    path.join(dir, "helix.json"),
    JSON.stringify({ $schema: "https://helixbiolab.github.io/helix-agent/config.json", ...config }),
  )
})
...
it.instance(
  "the standing objective is restated to the model on every turn",
  () =>
    Effect.gen(function* () {
      const { llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      const chat = yield* sessions.create({ title: "Campaign" })
      ...
      // The objective is keyed on the project directory, so it reaches the model even
      // though the user's message this turn was the single word "continue" — which is
      // exactly the case that would overwrite it if it were seeded from the step counter.
      const body = JSON.stringify((yield* llm.hits)[0]?.body)
      expect(body).toContain("campaign_objective")
      expect(body).toContain("repeat-containing proteins")
      expect(body).toContain("do not re-litigate")
    }),
  {
    init: (directory: string) =>
      Effect.promise(async () => {
        ...
        await fs.mkdir(p.join(directory, ".helix"), { recursive: true })
        await fs.writeFile(
          p.join(directory, ".helix", "objective.json"),
          JSON.stringify({
            statement: "Find repeat-containing proteins that are not yet catalogued.",
            decisions: ["predicted models do not count as prior annotation"],
...
it.instance("loop surfaces a truncated (length) finish instead of exiting silently", () =>
  Effect.gen(function* () {
    const { llm } = yield* useServerConfig(providerCfg)
    const events = yield* EventV2Bridge.Service
    const prompt = yield* SessionPrompt.Service
    const sessions = yield* Session.Service
    const chat = yield* sessions.create({ title: "Truncated" })
    const errors: NonNullable<SessionV1.Assistant["error"]>[] = []
    const expected = { name: "MessageOutputLengthError", data: {} } satisfies NonNullable<
    ...
    yield* llm.push(reply().text("half a sen").length())
    ...
    if (result.info.role === "assistant" && stored.info.role === "assistant") {
      ...
      // Previously this was treated as a clean finish: no error, no event, and the
      // turn just ended mid-sentence with nothing shown to the scientist.
...
it.instance("does not loop empty assistant turns for a simple reply", () =>
  Effect.gen(function* () {
    ...
    yield* llm.text("packages/helix/src/session/processor.ts")
  ...
  () =>
    Effect.gen(function* () {
      ...
      const other = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        model: { providerID: ProviderV2.ID.make("helix"), modelID: ModelV2.ID.make("kimi-k2.5-free") },
        noReply: true,
        parts: [{ type: "text", text: "hello" }],
      })

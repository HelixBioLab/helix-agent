...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".helix")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .helix/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const helix = path.join(test.directory, ".helix")
      const tool = path.join(helix, "tool")
  ...
  it.instance("ignores non-tool exports in .helix/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".helix", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-helix` was registering a tool
  ...
  it.instance("loads tools from .helix/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const helix = path.join(test.directory, ".helix")
      const tools = path.join(helix, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".helix", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const helix = path.join(test.directory, ".helix")
        const customTools = path.join(helix, "tools")
        const plugin = path.join(helix, "node_modules", "@helix", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(
            path.dirname(fileURLToPath(import.meta.resolve("zod"))),
            path.join(helix, "node_modules", "zod"),
            {
              dereference: true,
              recursive: true,
            },
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@helix/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".helix", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(helix, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(helix, "node_modules", "cowsay")

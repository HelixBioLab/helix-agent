...
// which buys cold verification for free: `helix verify` already re-hashes every
...
const res = await fetch(SOURCE, { headers: { "User-Agent": "helix/freeze-pipelines" } })
if (!res.ok) {
...
console.log(`  HELIX_NFCORE_PIPELINES=${path.relative(process.cwd(), manifestPath)} helix ...`)

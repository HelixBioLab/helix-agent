...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".bioinformatica")])),
})
...
describe("tool.registry", () => {
  ...
  withCodeMode.instance("exposes execute when code mode is enabled", () =>
    Effect.gen(function* () {
      ...
      const tools = yield* registry.tools({
        providerID: ProviderV2.ID.bioinformatica,
  ...
  it.instance("loads tools from .bioinformatica/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const bioinformatica = path.join(test.directory, ".bioinformatica")
      const tool = path.join(bioinformatica, "tool")
  ...
  it.instance("ignores non-tool exports in .bioinformatica/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".bioinformatica", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-bioinformatica` was registering a tool
  ...
  it.instance("loads tools from .bioinformatica/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const bioinformatica = path.join(test.directory, ".bioinformatica")
      const tools = path.join(bioinformatica, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".bioinformatica", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const bioinformatica = path.join(test.directory, ".bioinformatica")
        const customTools = path.join(bioinformatica, "tools")
        const plugin = path.join(bioinformatica, "node_modules", "@bioinformatica", "plugin")
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@bioinformatica/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@bioinformatica/plugin"',
              "export default tool({",
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".bioinformatica", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(bioinformatica, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@bioinformatica/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(bioinformatica, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(bioinformatica, "node_modules", "cowsay")

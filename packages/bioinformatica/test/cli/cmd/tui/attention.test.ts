...
import { createTuiAttention } from "@bioinformatica/tui/attention"
import type { TuiConfig } from "@bioinformatica/tui/config"
...
function config(attention: Partial<AttentionConfig["attention"]> = {}): AttentionConfig {
  return {
    attention: {
      ...
      sound: true,
      volume: 0.4,
      sound_pack: "bioinformatica.default",
      sounds: {},
      ...attention,
    },
  }
...
describe("createTuiAttention", () => {
  ...
  test("supports focused-only requests", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "Helix Agent", message: "focused" }])
  })
  ...
  test("notification can deliver while focused when requested", async () => {
    ...
    expect(audio.playCalls).toBe(1)
    expect(renderer.notifications).toEqual([{ title: "Helix Agent", message: "hello" }])
  })
  ...
  test("notifies while blurred", async () => {
    ...
    expect(await attention.notify({ title: "Helix Agent", message: "hello", sound: false })).toEqual({
      ok: true,
      notification: true,
      sound: false,
    })
  ...
  test("can play sound always while notification is blurred-only", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "Helix Agent", message: "hello again" }])
  })
  ...
  test("plays named sounds from the active sound pack", async () => {
    ...
    dispose()
    expect(attention.soundboard.current()).toBe("bioinformatica.default")
  })

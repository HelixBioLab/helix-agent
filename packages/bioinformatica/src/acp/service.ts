...
import type { AssistantMessage, Message, BioinformaticaClient, SessionMessageResponse } from "@bioinformatica/sdk/v2"
...
export const AuthMethodID = "bioinformatica-login"
...
export class Service extends Context.Service<Service, Interface>()("@bioinformatica/ACP/Service") {}

export function make(input: {
  sdk: BioinformaticaClient
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `bioinformatica auth login` in the terminal",
      name: "Login with Helix Agent",
      id: AuthMethodID,
    }
    ...
    if (params.clientCapabilities?._meta?.["terminal-auth"] === true) {
      authMethod._meta = {
        "terminal-auth": {
          command: "bioinformatica",
          args: ["auth", "login"],
          label: "Helix Agent Login",
        },
      }
    }
...
function makeDirectoryService(sdk: BioinformaticaClient) {
...
function makeUsageService(sdk: BioinformaticaClient) {
...
async function loadDirectorySnapshot(sdk: BioinformaticaClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, bioinformatica provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const bioinformaticaProvider = providers[ProviderV2.ID.make("bioinformatica")]
  const bioinformaticaModel = bioinformaticaProvider ? Provider.sort(Object.values(bioinformaticaProvider.models))[0] : undefined
  if (bioinformaticaProvider && bioinformaticaModel) return { providerID: bioinformaticaProvider.id, modelID: bioinformaticaModel.id }
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "Helix Agent prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Helix Agent service failure", service })
}
